            ...
            <div class="panel-body">
              ...
              <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('user.create') }}" class="brn btn-success btn-sm">New user</a>
                  </div>
                </div>
                  ...
                  <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td width="200" class="p-1">{{ date('F d, Y h:i A', strtotime($val->created_at)) }}</td>
                      ...
                      <td class="center hidden-phone p-1" width="100">
                          <a href="{{ route('user.manage', $val->id) }}" class="btn btn-sm btn-success"> <i
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4>{{__('user.delete_user')}}</h4>
        </div>
        <div class="modal-body">
          <strong>{{__('user.are_you_sure_to_delete_this_user')}}</strong>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">{{__('user.no')}}</button>
          <button type="button" class="btn btn-success btn-sm yes-btn">{{__('user.yes')}}</button>
        </div>
      </div>
    </div>
  </div>
