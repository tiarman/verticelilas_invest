            ...
            <div class="panel-body">
              ...
              <div class="row">
                <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                  <a href="{{ route('legal.practice.area.create') }}" class="brn btn-success btn-sm">Create new</a>
                </div>
              </div>
              ...
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="p-1 text-capitalize">{{ $val->description }}</td>
                    ...
                    <td class="text-capitalize p-1" width="100">
                      <div class="onoffswitch">
                        <input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox"
                               @if($val->status == \App\Models\LegalPracticeArea::$statusArray[0])
                               checked
                               @endif
                               data-id="{{ $val->id }}"
                    ...
                    <td class="center hidden-phone p-1" width="100">
                      <a href="{{ route('legal.practice.area.manage', $val->id) }}" class="btn btn-sm btn-success"> <i
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          {{-- <h4>{{__('Practi.delete_user')}}</h4> --}}
          <h4>Legal Practice Area</h4>
        </div>
        <div class="modal-body">
          {{-- <strong>{{__('WhoWeAre.are_you_sure_to_delete_this_user')}}</strong> --}}
          <strong>Are you sure delete Legal Practice area?</strong>
        </div>
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('change', 'input[name="onoffswitch"]', function () {
        ...
        $.ajax({
          url: "{{ route('ajax.update.legal.practice.status') }}",
          method: "post",
          dataType: "html",
      ...
      $(document).on('click', '.yes-btn', function () {
        ...
        $.ajax({
          url: "{{ route('legal.practice.area.destroy') }}",
          method: "delete",
          dataType: "html",
          data: {id: pid},
          success: function (data) {
