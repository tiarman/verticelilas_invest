        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">List of Attorney</h2>
            </header>
            <div class="panel-body">
              ...
              <div class="row">
                  <div class="col-lg-12 col-md-12 col-xl-12 text-right mb-3">
                    <a href="{{ route('attorney.create') }}" class="brn btn-success btn-sm">New Attorney</a>
                  </div>
                </div>
              ...
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                @foreach($data as $key => $val)
                  <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="p-1">{{ $val->link1 }}</td>
                    <td class="p-1">{{ $val->link2 }}</td>
                    <td class="p-1">{{ $val->link3 }}</td>
                    <td class="p-1">{{ $val->link4 }}</td>
                    <td width="200" class="p-1">{{ date('F d, Y h:i A', strtotime($val->created_at)) }}</td>
                      <td class="text-capitalize p-1" width="100">
                        <div class="onoffswitch">
                          <input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox"
                                 @if($val->status == \App\Models\attorney::$statusArray[0])
                                 checked
                                 @endif
                                 data-id="{{ $val->id }}"
                      ...
                      <td class="center hidden-phone p-1" width="100">
                          <a href="{{ route('attorney.manage', $val->id) }}" class="btn btn-sm btn-success"> <i
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4>Delete Attorney</h4>
        </div>
        <div class="modal-body">
          <strong>Are you sure delete attorney ?</strong>
        </div>
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('change', 'input[name="onoffswitch"]', function () {
        ...
        $.ajax({
          url: "{{ route('ajax.update.attorney.status') }}",
          method: "post",
          dataType: "html",
